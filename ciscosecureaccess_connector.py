# File: ciscosecureaccess_connector.py
#
...
from ciscosecureaccess_consts import (
    CISCOSECUREACCESS_CONNECTIVITY_ENDPOINT,
    CISCOSECUREACCESS_ERR_CONNECTIVITY_TEST,
    CISCOSECUREACCESS_SUCC_CONNECTIVITY_TEST,
)


class CiscosecureaccessConnector(BaseConnector):
    """
    Ciscosecureaccess connector class that serves as a starting point for new connectors.
    """
    ...
    def _handle_test_connectivity(self, param):
        ...
        endpoint = CISCOSECUREACCESS_CONNECTIVITY_ENDPOINT
        headers = {"Authorization": f"Bearer {self._api_key}"}

        ret_val, _ = self._make_rest_call(endpoint, action_result, headers=headers)

        if phantom.is_fail(ret_val):
            self.save_progress(CISCOSECUREACCESS_ERR_CONNECTIVITY_TEST)
            return action_result.get_status()

        self.save_progress(CISCOSECUREACCESS_SUCC_CONNECTIVITY_TEST)
...
if __name__ == "__main__":
    ...
    connector = CiscosecureaccessConnector()
